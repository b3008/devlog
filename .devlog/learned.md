...
- `devlog init`/hook-install should gitignore `.devlog/sessions.jsonl` (2026-06-14): the SessionEnd hook writes it into every installing project but nothing ignores it, so each repo accumulates an untracked runtime file. Fixed this repo's `.gitignore` by hand; tool-level fix is unbuilt — choose between editing the project `.gitignore` vs. a scoped `.devlog/.gitignore`.
