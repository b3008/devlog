...
- Version-aware upgrades (proposed 2026-06-12, undecided): `__version__` has been 0.1.0 since April while the manifest dutifully records it — version discipline is absent, so installs can't say what they're from. Proposed slice: bump to 0.2.0, stamp the sentinel (`<!-- DEVLOG:START v0.2.0 ... -->`, backward compatible — the marker is a prefix match), and have `devlog status` compare manifest hashes against shipped templates to report drift. Recommended against hook-driven auto-resync (mid-session CLAUDE.md mutation, surprise VCS diffs, CLI-on-PATH assumption). Reinstall-as-upgrade is already safe via today's carry-forward/hashing.
